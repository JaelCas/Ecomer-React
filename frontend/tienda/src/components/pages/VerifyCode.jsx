...
export default function VerifyCode() {
    ...
    function handleCodeChange(index, value) {
        ...
        //mueve el foco a la siguiente caja si escribio algo
        if (value && index < 5) {
            inputsRef.current[index + 1]?.focus();
        }
    }
    //cuando el usuario presiona backspace en caja vacia, retrocede
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex justify-center gap-2"
                                onPaste={handlePaste}>
                                    {code.map((digit, index) => (
                                        ...
                                        className="w-12 h-14 text-center text-2xl font-bold border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900"
                                        />
                                    ))}
                            </div>
                        </div>
                                ...
                                <button type="button"
                                 onClick={() => setShowConfirm(!showConfirm)}
                                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                    {showConfirm ? <EyeOff className="w-5 h-5"/>
                                    : <Eye className="w-5 h-5"/>}
                                </button>
                            </div>  
                        </div>
                        ...
                        <button type="submit" disabled={loading}
                            className="w-full bg-linear-to-r from-blue-600 to-purple-600 text-white py-3 rounded-lg font-semibold disabled:opacity-50 flex items-center justify-center">
                            {loading ? (
